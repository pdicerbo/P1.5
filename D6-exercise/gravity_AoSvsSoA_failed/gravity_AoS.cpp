...
template <typename T>
void UpdateAccelerations( planet< T >* state, int num)
{
	T den;
	for (int i=0; i<num ; i++)
		{
...
template <typename T>
void UpdatePositions( planet< T >* state, T dt, int num)
{
	for (int i=0; i<num ; i++)
                {
...
template <typename T>
void UpdateVelocities( planet< T >* state, T dt, int num)
{
        for (int i=0; i<num; i++)
                {
...
template <typename T>
void evolve( planet< T >* state, T dt, int num)
{

	UpdateAccelerations<T>(state, num);
	UpdatePositions<T>(state, dt, num);
	UpdateVelocities<T>(state, dt, num);
}
...
template <typename T>
void print_out(ofstream& output,  planet< T >* state, int num)
{
...
*/
	...
	for (int t=0; t<steps; t++)
		...
		if (t%100==0)
			{
			print_out<double>(output, state, num);
			}
#endif
		evolve<double>(state, dt, num);
		}
	t1=seconds()-t1;
